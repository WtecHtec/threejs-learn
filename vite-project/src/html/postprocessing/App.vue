...
onMounted(() => {
	...
	const portalMaterial = new THREE.ShaderMaterial({
		uniforms: {
			...
			perlinnoise: {
				type: 't',
				value: textureLoader.load('./static/images/perlinnoise.png'),
			},
			sparknoise: {
				type: 't',
				value: textureLoader.load('./static/images/sparknoise.png'),
			},
			waterturbulence: {
				type: 't',
				value: textureLoader.load('./static/images/waterturbulence.png'),
			},
			noiseTex: {
				type: 't',
				value: textureLoader.load('./static/images/noise.png'),
			},
			color5: {
	...
	// 加载模型
	let model = null;
	loader.load('./static/models/rickAndMorty.glb', mesh => {
		if (mesh.scene) {
